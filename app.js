...
let M = null, OV = {}, WHOLE = {};
const state = { i0: 0, i1: 1, on: new Set(), R: 300000, ymin: 0, ymax: 1.08, mode: 'individual', fnu: false, bb: false, form: 'off', tell: false, snr: 0, ppre: 3, seed: (Math.random() * 4294967296) >>> 0, x: 0, Rwant: 300000 };  // snr 0 = off
...
/* The slider stops at whatever the data on show was computed at.  The five
   stars are native R = 300,000; the temperature sequence ships at 50,000, and
   offering more would be claiming a resolution it does not have. */
const rMax = () => (typeof M !== 'undefined' && M && M.resolution) || R_NATIVE;
const posToR = p => Math.round(10 ** (Math.log10(R_MIN)
  + (p / 1000) * (Math.log10(rMax()) - Math.log10(R_MIN))));
const rToPos = R => Math.round(1000 * (Math.log10(R || rMax()) - Math.log10(R_MIN))
  / (Math.log10(rMax()) - Math.log10(R_MIN)));
...
const STARS = [
  ...
  // no spectral type: these are not stars.  The held parameters are in the
  // hover text, and the varying one is on the rail's slider.
  { name: 'Teff Sequence', sp: '', teff: 5650, logg: 5.0, feh: 0.0, afe: 0.0,
    dir: 'data/tgrid', seq: true },
  { name: 'log g Sequence', sp: '', teff: 4500, logg: 4.0, feh: 0.0, afe: 0.0,
    dir: 'data/logg4500', seq: true },
];
let DB = STARS[0].dir;
function fail(err) {
  console.error(err);
...
/* ---------------------------------------------------------------------
   ...
   One parameter varies.  The browser holds the two models bracketing the
   slider and interpolates between them in log T, which is the abscissa the
   held-out tests preferred.  The two quantities are treated differently
   because they behave differently: the normalized flux is interpolated
   LINEARLY (rms 0.0084 against 0.0097 for log-linear at dlogT = 0.05 -- lines
   saturate, so residual flux is not the exponential a single line's opacity
   is), while the continuum is interpolated in the LOG, where it spans 7000x
   across the sequence and costs 1.1% instead of 4.4%.  _cont ships as log10
   already, so lerping the stored value IS the log interpolation.
   ------------------------------------------------------------------ */
const TG = {};                       // raw models, keyed by dir + model key

async function seqModel(m) {
  const id = DB + '/' + m.key;
  if (TG[id]) return TG[id];
  if (INFLIGHT[id]) return INFLIGHT[id];
  INFLIGHT[id] = (async () => {
    const [n, c] = await Promise.all([
      fetch(`${DB}/${m.key}_norm.bin`).then(r => r.ok ? r.arrayBuffer() : null),
      fetch(`${DB}/${m.key}_cont.bin`).then(r => r.ok ? r.arrayBuffer() : null)]);
    if (!n || !c) return null;
    const dec1 = async b => { const u = new Uint8Array(b), h = parseHdr(u);
      return h ? decodeAll(u, h) : null; };
    const [nn, cc] = await Promise.all([dec1(n), dec1(c)]);
    if (nn && cc) TG[id] = { norm: nn, cont: cc };
    return TG[id];
  })().catch(() => null).finally(() => { delete INFLIGHT[id]; });
  return INFLIGHT[id];
}

/* Teff is walked in the log, since that is the axis its grid is even on and
   the abscissa the held-out test preferred.  log g is already a logarithm, so
   it is walked linearly. */
const isLog = () => M.axis === 'teff';
const ax = v => (isLog() ? Math.log10(v) : v);
const axInv = v => (isLog() ? Math.pow(10, v) : v);
const xs = () => M.models.map(m => m.x);
const xLo = () => xs()[0];
const xHi = () => xs()[M.models.length - 1];
const snapX = v => (isLog() ? Math.round(v / 10) * 10 : Math.round(v * 100) / 100);
const posToX = p => snapX(axInv(ax(xLo()) + (p / 1000) * (ax(xHi()) - ax(xLo()))));
const xToPos = v => Math.round(1000 * (ax(v) - ax(xLo())) / (ax(xHi()) - ax(xLo())));
const fmtX = v => (isLog() ? `${fmt(v)} K` : v.toFixed(2));

function seqBracket(v) {
  const t = xs();
  let i = 0;
  while (i < t.length - 2 && t[i + 1] < v) i++;
  return [t[i], t[i + 1]];
}
...
function syncSeq() {
  ...
  $('#spsec').hidden = on;
  if (!on) return;
  $('#axlabel').innerHTML = M.axis === 'teff' ? 'T<sub>eff</sub>' : 'log <i>g</i>';
  $('#tefflabel').textContent = fmtX(state.x);
  /* The held parameter reads the same way whichever axis is moving: a label
     and the value it is pinned at.  Gravity has three temperatures to choose
     between; temperature has one gravity, which is still worth showing as a
     card so the two sequences look like the same instrument. */
  const f = M.fixed;
  $('#heldlabel').innerHTML = M.axis === 'teff' ? 'at log <i>g</i>' : 'at T<sub>eff</sub>';
  const opts = M.axis === 'teff'
    ? [{ v: f.logg, txt: num(f.logg), t: null }]
    : [3500, 4500, 5500].map(t => ({ v: t, txt: String(t), t }));
  $('#heldbtns').innerHTML = opts.map(o =>
    `<button class="seg tgt${o.v === (M.axis === 'teff' ? f.logg : f.teff) ? ' on' : ''}"`
    + `${o.t ? ` data-t="${o.t}"` : ''} title="${M.axis === 'teff'
        ? `Held at log g ${num(f.logg)}` : `Vary gravity at ${o.txt} K`}">${o.txt}</button>`
  ).join('');
  const card = [...document.querySelectorAll('.star')].find(b2 => b2.dataset.seq);
  if (card) card.title = M.axis === 'teff'
    ? `Teff Sequence \u2014 Teff ${fmt(state.x)} K, log g ${num(f.logg)}, `
      + `[Fe/H] ${rat(f.feh)}, [\u03b1/Fe] ${rat(f.afe)}`
    : `log g Sequence \u2014 log g ${state.x.toFixed(2)}, Teff ${fmt(f.teff)} K, `
      + `[Fe/H] ${rat(f.feh)}, [\u03b1/Fe] ${rat(f.afe)}`;
}

/* Build _norm and _cont for the current slider position, in the same uint16
   form every other star ships, so nothing downstream needs to know. */
async function seqBuild() {
  const [x0, x1] = seqBracket(state.x);
  const m0 = M.models.find(m => m.x === x0), m1 = M.models.find(m => m.x === x1);
  const [A, B] = await Promise.all([seqModel(m0), seqModel(m1)]);
  if (!A || !B) return false;
  const w = (ax(state.x) - ax(x0)) / (ax(x1) - ax(x0));
  const n = M.n, u = 1 - w;
  const nrm = new Uint16Array(n), cnt = new Uint16Array(n);
  const [clo, chi] = M.cont_log;
  const cmax = Math.exp(u * Math.log(m0.cmax) + w * Math.log(m1.cmax));
  M.flux_max = cmax;
  const k = (chi - clo) / 65535, f = 65535 / cmax;
  for (let i = 0; i < n; i++) {
    nrm[i] = (u * A.norm[i] + w * B.norm[i] + 0.5) | 0;
    // the stored value is log10(continuum); lerp it, then come back to linear
    const lg = clo + (u * A.cont[i] + w * B.cont[i]) * k;
    const v = Math.pow(10, lg) * f;
    cnt[i] = v > 65535 ? 65535 : (v + 0.5) | 0;
  }
  WHOLE['_norm'] = nrm; WHOLE['_cont'] = cnt;
  // _flux is derived everywhere else by getWhole, which ensure() skips for a
  // sequence -- so build it here or the top panel has only its continuum
  WHOLE['_flux'] = fluxFrom(nrm, cnt);
  for (const key of Object.keys(DECODED)) if (key !== TELL) delete DECODED[key];
...
function draw() {
  ...
  const useFull = M.kind === 'seq' || (state.i1 - state.i0) <= FULL_MAX;
  let y = padT;
  const FS = 15;
...
function drawBrush() {
  ...
  const e = envelope('_norm', 0, M.n, NB, M.kind === 'seq');
...
async function ensure() {
  if (M.kind === 'seq') {
    await seqBuild();
    if (state.tell) { await loadOv(TELL); await getWhole(TELL); }
    return;
  }
...
function writeHash() {
  ...
  if (M.kind === 'seq') p.push('x=' + state.x);
...
function readHash() {
  ...
  if (h.has('T')) state.x = +h.get('T');         // links written before the rename
...
/* Fifty-one identical pills is a list you read; five named families is a list
   you scan.  The families come from meta.json, so they follow the species
   table rather than being written out here. */
const GROUP_LABEL = { 'light elements': 'Light elements', 'iron peak': 'Iron peak',
  ions: 'Ions', 'neutron capture': 'Neutron capture', molecules: 'Molecules' };

function buildChips() {
  const box = $('#species');
  box.innerHTML = '';
  for (const gname of M.groups) {
    const members = M.series.filter(q => q.group === gname);
    if (!members.length) continue;
    // grid-column 1/-1 takes a whole row, so each family starts on a fresh one
    // however the last one happened to fall across the three columns
    const h = document.createElement('div');
    h.className = 'grphdr';
    h.textContent = GROUP_LABEL[gname] || gname;
    box.appendChild(h);
    for (const s of members) {
      const el = document.createElement('label');
      el.className = 'chip' + (s.kind === 'mol' ? ' mol' : '') + (state.on.has(s.name) ? ' on' : '');
      el.innerHTML = `<span class="sw"></span>${s.name}`;
      // the rail is a wall of names otherwise; this says which ones matter
      // in the star on view, which is a different list for a G dwarf and an
      // M dwarf.  absorption is 1 - mean(F/F_cont) for that species alone.
      const pc = s.absorption * 100;
      el.title = `${s.name}: removes ${pc < 0.01 ? '<0.01' : pc.toFixed(2)}% `
               + 'of the flux, averaged over the band';
...
function buildStars() {
  ...
  STARS.forEach((st, i) => {
    ...
    // one card covers all three log g roots
    const on = st.dir === DB || (st.dir.startsWith('data/logg')
                                 && DB.startsWith('data/logg'));
    b.className = 'star' + (on ? ' on' : '');
    b.setAttribute('role', 'radio');
    b.setAttribute('aria-checked', on ? 'true' : 'false');
    // the Sun's numbers come from the synthesis, not from this table
    const m = on && M ? M.model : st;
    if (st.seq) b.dataset.seq = '1';
    b.title = `${st.name}${st.sp ? ` (${st.sp})` : ''} \u2014 Teff ${m.teff} K, `
            + `log g ${num(m.logg)}, [Fe/H] ${rat(m.feh)}, `
            + `[\u03b1/Fe] ${rat(m.afe || 0)}`;
...
async function selectStar(i) {
  if (STARS[i].dir === DB) return;
  await loadRoot(STARS[i].dir);
}

/* The three log g sequences are three data roots that share one card, so the
   reload is factored out of selectStar rather than duplicated for them. */
async function loadRoot(dir) {
  ...
  // Keep the WAVELENGTH window, not the fraction of the grid: the sequences
  // are 129,671 points over 3005-10990 A while the stars are 585,579 over
  // 3549-24993, so the same fraction is a different part of the spectrum and
  // switching used to jump the view somewhere else entirely.
  const a0 = idxAir(state.i0), a1 = idxAir(state.i1);
  M = await (await fetch(`${DB}/meta.json`)).json();
  let i0 = airIdx(Math.max(a0, M.lam_air_min));
  let i1 = airIdx(Math.min(a1, M.lam_air_max));
  if (!(i1 - i0 >= 40)) { i0 = 0; i1 = M.n; }     // no overlap: show it all
  state.i0 = Math.max(0, i0); state.i1 = Math.min(M.n, i1);
  if (pristine) {
    state.on.clear();
    M.default_on.forEach(n => state.on.add(n));
  }
  if (M.kind === 'seq') {
    if (!state.x || state.x < xLo() || state.x > xHi()) state.x = M.model[M.axis];
    state.x = Math.min(Math.max(state.x, xLo()), xHi());
    $('#teff').value = xToPos(state.x);
  }
  state.R = Math.min(state.Rwant, rMax());
  $('#res').value = rToPos(state.R);
  buildStars(); buildChips(); loadLineIndex(); syncSeq();
  deferHash();
  await refresh();
}
...
async function main() {
  ...
  $('#res').addEventListener('input', e => {
    state.R = state.Rwant = posToR(+e.target.value);
    syncSnr();
  ...
  $('#teff').addEventListener('input', e => {
    state.x = posToX(+e.target.value);
    syncSeq(); deferHash(); refresh();           // a new bracket may need fetching
  });
  // the three log g sequences are three data roots; switching between them is
  // a star switch in everything but name.  Delegated, because syncSeq rebuilds
  // these buttons whenever the axis changes.
  $('#heldbtns').addEventListener('click', ev => {
    const b = ev.target.closest('.seg.tgt[data-t]');
    if (b && DB !== `data/logg${b.dataset.t}`) loadRoot(`data/logg${b.dataset.t}`);
  });
